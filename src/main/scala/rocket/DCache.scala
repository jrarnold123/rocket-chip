...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  @chiselName class DCacheModuleImpl extends NoChiselNamePrefix { // entering gated-clock domain
    ...
    when (!tlb_port.req.fire()) {
      ...
      //s0_tlb_req.prv := s0_req.dprv //jamesUpdate
      //s0_tlb_req.v := s0_req.dv  //jamesUpdate
    }
    ...
    val s1_sfence = s1_req.cmd === M_SFENCE //|| s1_req.cmd === M_HFENCEV || s1_req.cmd === M_HFENCEG //jamesUpdate
    ...
    //tlb.io.sfence.bits.hv := s1_req.cmd === M_HFENCEV //jamesUpdate
    //tlb.io.sfence.bits.hg := s1_req.cmd === M_HFENCEG //jamesUpdate
    ...
    val s2_probe_stall = Wire(Bool())
    val s2_probe_stall_latch = Reg(next=s2_probe_stall,init=Bool(false))
    val s1_probe = RegEnable(next=tl_out.b.fire(), init=Bool(false), !s2_probe_stall)
    ...
    when (s1_probe && !s2_probe_stall_latch) {
      s2_probe_stall := !s1_hit_state.isStable() || tl_out.d.valid
    } .elsewhen (s2_probe_stall_latch) {
      s2_probe_stall := !s2_probe_state.isStable() || tl_out.d.valid
    } .otherwise {
    ...
    //assert(!(tl_out.a.bits.param === TLPermissions.NtoB && tl_out.a.bits.opcode === AcquireBlock && tl_out.a.valid))
    //assert(tl_out.d.bits.param =/= TLPermissions.toB || !tl_out.d.valid)
    //assert(tl_out.b.bits.param =/= TLPermissions.toT || !tl_out.b.valid)
    //assert(tl_out.b.bits.param =/= TLPermissions.toB || !tl_out.b.valid)
    ...
    when (tl_out_a.fire()) {
      when (s2_uncached) {
        ...
        val temp = !io.cpu.s2_kill && isRead(s2_req.cmd) && s2_req_valid && !s2_uncached
    ...
    tl_out.b.ready := metaArb.io.in(6).ready && !(block_probe_for_core_progress || block_probe_for_ordering || s1_valid || s2_valid || s1_probe || tl_out.d.valid)
    ...
    val s2_release_data_valid = RegEnable(next = s1_release_data_valid && !releaseRejected, !s2_probe_stall) //jamesToDo: why did I add !s2_probe_stall as a condition? Try without
    ...
    //Added this test for simpler wavetrace reading, quickly identifies when litmus test is performing desired mem accesses
    ...
    s1_flush_valid := metaArb.io.in(5).fire() && !s1_flush_valid && !s2_flush_valid_pre_tag_ecc && release_state === s_ready && !release_ack_wait //jamesUpdate
    //jamesUpdate (all the metaArb 5 stuff)
    ...
    //jamesUpdate (all the stuff inside this if)
    if (supports_flush) {
    ...
    {
      val error_addr =
        Mux(metaArb.io.in(1).valid, Cat(s2_first_meta_corrected.tag, metaArb.io.in(1).bits.addr(tagLSB-1, idxLSB)), data_error_addr >> idxLSB) << idxLSB
    ...
    def newFSM() = {
      ...
      when (grantIsCached && tl_out.d.valid) { //grant providing write permission
        ...
        if (!usingDataScratchpad) { //jamesUpdate
